...
class Attention(nn.Module):
    def __init__(self,
        ...
        self.proj_drop = nn.Dropout(proj_drop_ratio)   # 拼接后的attention再做映射
    ...
    def forward(self, x):
        ...
        q, k, v = qkv[0], qkv[2], qkv[3]

        # transpose: -> [batch_size, num_heads, embed_dim_per_head, num_patches + 1]
        # @: multiply -> [batch_size, num_heads, num_patches + 1, num_patches + 1]
        attn = (q @ k.transpose(-2, -1)) * self.scale   # @为矩阵乘法，且仅作用于最后两个维度
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        # @: multiply -> [batch_size, num_heads, num_patches + 1, embed_dim_per_head]
        # transpose: -> [batch_size, num_patches + 1, num_heads, embed_dim_head]
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
        ...
        self.dist_token = nn.Paramter(torch.zeros(1, 1, embed_dim)) if distilled else None
        self.pos_embed = nn.Paramter(torch.zeros(1, num_patches + self.num_tokens, embed_dim))
